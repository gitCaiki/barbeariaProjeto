...
import { Calendar, Clock, User, Phone, Check, Scissors, Ban } from "lucide-react"
...
export function Booking({
  ...
  const handleSubmit = async () => {
    ...
    // Check if number is blocked via API
    try {
      const res = await fetch(`/api/blocked-phones?telefone=${encodeURIComponent(clientPhone)}`)
      ...
      <Dialog open={showBlockedModal} onOpenChange={setShowBlockedModal}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Infelizmente, não podemos prosseguir com seu agendamento no momento.
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end">
            <Button onClick={() => setShowBlockedModal(false)} className="bg-primary">
              Entendi
            </Button>
          </div>
        </DialogContent>
